...
config = {
   # "v2.9.7": {"path": "/scratch/jwatts/mucol/data/reco/electronGun_pT_0_50/electronGun_pT_0_50_reco_5.slcio", "trk_coll": "SiTracks_Refitted"},
    "v11": {"path": "/scratch/jwatts/mucol/data/reco/electronGun_pT_0_50/electronGun_pT_0_50_reco_7.slcio", "trk_coll": "SelectedTracks"}
}
...
print(f"\n{'-'*10}\nFINAL AVERAGES (E_true > 10, E_reco > 2)\n{'-'*10}")
for v in config:
    n = stats[v]["evts"]
    if n > 0:
...
plotHelper.plotHistograms(res_map, f"{out_dir}/energy_response.svg", xlabel="(E_reco - E_true) / E_true", ylabel="Events", atltext=["Muon Collider", "Comparison"])

for feat in ["fail_eta", "fail_e"]:
    f_map = {v: hists[v][feat] for v in config}
    plotHelper.plotHistograms(f_map, f"{out_dir}/{feat}.svg", xlabel=feat, ylabel="Failure Counts", atltext=["Muon Collider", "Failures Only"])
