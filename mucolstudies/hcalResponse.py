...
# Make 2D plot and a TProfile to understand response v HCAL fraction
...
for s in samples:
    for i in range(len(energy_bins) - 1):
        ...
        h2[s][i].Draw("colz")
        h2[s][i].GetXaxis().SetTitle("HCAL Fraction")
        h2[s][i].GetYaxis().SetTitle("Response")
        c.SaveAs(os.path.join(PLOT_DIR, f"RESPONSE_VS_HCALFRAC_{s}_{name}.png"))
        c = ROOT.TCanvas("cresp2dprof", "cresp2dprof")
        h_prof = h2[s][i].ProfileX(f"pfx_{s}_{name}")
        h_prof.SetTitle("")
        h_prof.GetXaxis().SetTitle("HCAL Fraction")
        h_prof.GetYaxis().SetTitle("Response")
        h_prof.Draw("hist")
        h_prof.SetMinimum(0)
        h_prof.SetMaximum(2)
        ...
        c.SaveAs(os.path.join(PLOT_DIR, f"RESPONSE_VS_HCALFRAC_{s}_{name}_prof.png"))
        c.Close()

# per sample: overlay of profiles (response vs HCAL fraction) for its energy bins
for s in samples:
    p_map = {name: p for name, p in prof[s].items() if p.GetEntries() > 0}
    if len(p_map) == 0:
        continue
    c = ROOT.TCanvas("can", "can")
    keys = list(p_map.keys())
    for j, k in enumerate(keys):
        p_map[k].SetLineColor(colors[j % len(colors)])
        p_map[k].SetLineWidth(2)
        if j == 0:
            p_map[k].SetTitle("")
            p_map[k].GetXaxis().SetTitle("HCAL Fraction")
            p_map[k].GetYaxis().SetTitle("Response")
            p_map[k].SetMinimum(0)
            p_map[k].SetMaximum(2)
            p_map[k].Draw("hist")
        else:
            p_map[k].Draw("hist same")
    leg = ROOT.TLegend(0.68, 0.65, 0.93, 0.88)
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.SetTextSize(0.035)
    for k in keys:
        leg.AddEntry(p_map[k], k, "l")
    leg.Draw()
    c.SaveAs(os.path.join(PLOT_DIR, f"RESPONSE_VS_HCALFRAC_PROFILE_{s}.png"))
    c.Close()
...
for s in samples:
    ...
    maxy = 1.3*max(r_map[k].GetMaximum() for k in keys)
    for j, k in enumerate(keys):
        r_map[k].SetLineColor(colors[j % len(colors)])
        r_map[k].SetLineWidth(2)
        if j == 0:
            r_map[k].SetTitle("")
            r_map[k].GetXaxis().SetTitle("(E_{meas} - E_{true})/E_{true}")
            r_map[k].GetYaxis().SetTitle("Counts")
            r_map[k].SetMinimum(0)
            r_map[k].SetMaximum(maxy)
            r_map[k].Draw("hist")
        else:
            r_map[k].Draw("hist same")
    leg = ROOT.TLegend(0.68, 0.65, 0.93, 0.88)
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.SetTextSize(0.035)
    for k in keys:
        leg.AddEntry(r_map[k], k, "l")
    leg.Draw()
    c.SaveAs(os.path.join(PLOT_DIR, f"ERES_{s}.png"))
    c.Close()
