...
samples       = glob.glob("/scratch/jwatts/mucol/v2.11/reco_v2/pions_0_50/*.slcio")
...
h_denom = ROOT.TH1F("mcp_pi_eta",   ";#eta;Counts", 50, -2.5, 2.5)
h_match = ROOT.TH1F("match_pi_eta", ";#eta;Counts", 50, -2.5, 2.5)
h_soup  = ROOT.TH1F("soup_pi_eta",  ";#eta;Counts", 50, -2.5, 2.5)
h_pand  = ROOT.TH1F("pand_pi_eta",  ";#eta;Counts", 50, -2.5, 2.5)
...
for f in files:
    ...
    for event in reader:
        ...
        for mcp_pi in mcp_pions:
            ...
            in_cone = [(tlv, pfo) for tlv, pfo in good_pfos if mcp_pi.DeltaR(tlv) < dr_match]
            if len(in_cone) == 0:
                continue
            best_tlv, best_pfo = max(in_cone, key=lambda x: x[1].getEnergy())

            h_match.Fill(mcp_pi.Eta())

            if passSoup(best_pfo):
                h_soup.Fill(mcp_pi.Eta())
            if abs(best_pfo.getType()) == 11:
                h_pand.Fill(mcp_pi.Eta())

    reader.close()

print(f"\nProcessed {n_events} events")

print("\n--- Fake Rate Results (pion passing electron ID) ---")
n_den = h_denom.GetEntries()
n_mat = h_match.GetEntries()
for name, h in [("Electron soup", h_soup), ("Pandora ID=11", h_pand)]:
    n_num = h.GetEntries()
    fake_all = n_num/n_den if n_den > 0 else 0
    fake_mat = n_num/n_mat if n_mat > 0 else 0
    print(f"{name:<15} | per truth pion: {fake_all:.4f} ({int(n_num)}/{int(n_den)}) | per trk+clus PFO: {fake_mat:.4f} ({int(n_num)}/{int(n_mat)})")

#fake_map = {}
#fake_map["Electron soup"] = ROOT.TEfficiency(h_soup, h_match).CreateGraph()
#fake_map["Pandora ID=11"] = ROOT.TEfficiency(h_pand, h_match).CreateGraph()

#plotEfficiencies(fake_map, os.path.join(PLOT_DIR, "FAKERATE_SOUP_ETA.svg"),
#                 xlabel="#eta", ylabel="Fake rate")
